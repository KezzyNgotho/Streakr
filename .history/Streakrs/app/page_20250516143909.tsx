...
import { mintNFT } from "../lib/streakrContract";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex">
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarExpanded ? "ml-16 md:ml-40" : "ml-16"} w-full ${darkMode ? "bg-gray-900 text-white" : ""} pt-14 pb-16 md:pb-0`}> 
        <main className="flex-1 px-2 sm:px-4 pb-16 md:pb-0">
...
function HomePage() {
  ...
  async function handleMint(streakId: string) {
    setMinted((prev) => [...prev, streakId]);
    // Call NFT mint contract
    try {
      if (!address) throw new Error("Wallet not connected");
      // You can customize the URI as needed
      const tx = await mintNFT("https://placehold.co/400x400?text=Streakr+NFT", address as string);
      alert("Mint transaction sent! " + tx);
    } catch (err: unknown) {
  ...
  const topRow = (
    <div className="flex flex-col sm:flex-row items-center mt-2 mb-4 w-full gap-2 sm:gap-0 justify-between">
      {/* Welcome card left, flex-1 to fill space */}
      <div className="flex-1 flex items-center gap-3 sm:gap-4 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-2xl shadow-lg px-3 sm:px-5 py-2 sm:py-3 w-full">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white flex items-center justify-center text-2xl sm:text-3xl shadow border-2 border-white">{avatar}</div>
        <div className="flex-1">
          <div className="text-base sm:text-lg font-bold text-gray-900">Welcome to <span className="text-primary">Streakrs!</span></div>
        ...
        <div className="flex flex-col gap-2 sm:gap-6">
          ...
          <section className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-orange-400/20 text-xs">
            ...
            <div className="flex gap-2 sm:gap-3 flex-wrap">
              ...
              {minted.map((id) => (
                <motion.div key={id} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center bg-orange-50 rounded-xl shadow p-2 sm:p-3 w-16 sm:w-20 border border-gray-100">
