...
import { Home as LucideHome, Search, Image as LucideImage, Trophy, Settings as LucideSettings } from "lucide-react";

type HomePageProps = { showWelcomeCardOnly?: boolean };

// Sidebar nav items config
const SIDEBAR_NAV = [
  { key: "home", icon: <LucideHome size={24} />, label: "Home" },
  { key: "explore", icon: <Search size={24} />, label: "Explore" },
  { key: "gallery", icon: <LucideImage size={24} />, label: "Collectibles" },
  { key: "leaderboard", icon: <Trophy size={24} />, label: "Leaderboard" },
];
const SIDEBAR_BOTTOM = [
  { key: "settings", icon: <LucideSettings size={24} />, label: "Settings" },
];

function Sidebar({ expanded, setExpanded, activeTab, setActiveTab }: {
  expanded: boolean;
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen z-50 transition-all duration-300
        ${expanded ? "w-56" : "w-16"}
        bg-white/60 backdrop-blur-xl shadow-2xl border-r border-gray-200 flex flex-col
        items-center py-4 px-2 overflow-hidden relative`}
      style={{ transitionProperty: 'width,background-color' }}
    >
      {/* Gradient overlay for premium glass look */}
      <div className="pointer-events-none absolute inset-0 z-0 rounded-r-3xl" style={{background: 'linear-gradient(160deg,rgba(0,82,255,0.08) 0%,rgba(255,255,255,0.0) 100%)'}} />
      {/* Logo & expand/collapse */}
      <div className="flex items-center w-full mb-8 select-none z-10">
        <span className="text-3xl font-extrabold text-primary tracking-tight mr-2">🔥</span>
        {expanded && <span className="text-lg font-bold text-gray-900">Streakrs</span>}
        <button
          className="ml-auto p-1 rounded hover:bg-primary/10 transition"
      ...
      {/* Nav items */}
      <nav className="flex-1 flex flex-col gap-1 w-full items-center z-10">
        {SIDEBAR_NAV.map(tab => (
          <button
            key={tab.key}
            aria-label={tab.label}
            className={`group relative flex items-center w-12 ${expanded ? "md:w-[90%]" : "w-12"} py-3 my-1 rounded-xl focus:outline-none transition-all overflow-visible
              ${activeTab === tab.key ? "bg-primary/10 text-primary font-bold shadow-lg" : "text-gray-500 hover:bg-primary/10"}`}
            onClick={() => setActiveTab(tab.key)}
            tabIndex={0}
          >
            {/* Animated active indicator bar */}
            <span className={`absolute left-1 top-2 bottom-2 w-1 rounded-full transition-all duration-300
              ${activeTab === tab.key ? "bg-gradient-to-b from-primary to-secondary shadow-lg scale-y-110" : "bg-transparent"}`} />
            <span className="text-2xl mx-auto transition-all flex items-center justify-center">{tab.icon}</span>
            {expanded && <span className="ml-4 text-sm font-medium transition-all">{tab.label}</span>}
        ...
        {/* Bottom nav */}
        {SIDEBAR_BOTTOM.map(tab => (
          <button
            key={tab.key}
            aria-label={tab.label}
            className={`group relative flex items-center w-12 ${expanded ? "md:w-[90%]" : "w-12"} py-3 my-1 rounded-xl focus:outline-none transition-all overflow-visible
              ${activeTab === tab.key ? "bg-gray-900/10 text-gray-900 font-bold shadow-lg" : "text-gray-500 hover:bg-gray-900/10"}`}
            onClick={() => setActiveTab(tab.key)}
            tabIndex={0}
          >
            <span className={`absolute left-1 top-2 bottom-2 w-1 rounded-full transition-all duration-300
              ${activeTab === tab.key ? "bg-gradient-to-b from-gray-900 to-primary shadow-lg scale-y-110" : "bg-transparent"}`} />
            <span className="text-2xl mx-auto transition-all flex items-center justify-center">{tab.icon}</span>
            {expanded && <span className="ml-4 text-sm font-medium transition-all">{tab.label}</span>}
            {!expanded && (
              <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none">{tab.label}</span>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex">
      <Sidebar expanded={sidebarExpanded} setExpanded={setSidebarExpanded} activeTab={activeTab} setActiveTab={setActiveTab} />
      <div className={`flex-1 transition-all duration-300 ${sidebarExpanded ? "ml-16 md:ml-56" : "ml-16"} w-full max-w-7xl mx-auto`}> 
