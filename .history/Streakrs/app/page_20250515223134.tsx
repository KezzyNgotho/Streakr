          ...
          {[
            ...
            <button
              ...
              className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
                ${activeTab === tab.key ? `${tab.color} font-bold bg-white/90 shadow-lg` : "text-gray-500 hover:text-primary/80"}`}
              onClick={() => setActiveTab(tab.key)}
              tabIndex={0}
            >
              <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === tab.key ? "scale-125" : "scale-100"}`}>{tab.icon}</span>
              <span className={`text-xs font-medium transition-all ${activeTab === tab.key ? "" : "opacity-80"} hidden md:block`}>{tab.label}</span>
              {/* Notification badge for Profile (stub) */}
              {tab.key === "profile" && (
                <span className="absolute top-3 right-6 w-2 h-2 bg-red-500 rounded-full border-2 border-white animate-pulse" />
              )}
            </button>
          ))}
        </div>
