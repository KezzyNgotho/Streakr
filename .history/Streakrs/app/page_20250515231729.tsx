  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen z-50 transition-all duration-300
        ${expanded ? "w-56" : "w-16"}
        bg-white/60 backdrop-blur-xl border-r border-gray-200 flex flex-col
      ...
      <nav className="flex-1 flex flex-col gap-1 w-full items-center">
        {SIDEBAR_NAV.map(tab => (
          <button
            ...
            className={`group relative flex items-center w-12 ${expanded ? "md:w-[90%]" : "w-12"} py-3 my-1 rounded-xl focus:outline-none transition-all
              ${activeTab === tab.key ? "bg-primary/10 text-primary font-bold" : "text-gray-500 hover:bg-primary/10"}`}
        ...
        {SIDEBAR_BOTTOM.map(tab => (
          <button
            ...
            aria-label={tab.label}
            className={`group relative flex items-center w-12 ${expanded ? "md:w-[90%]" : "w-12"} py-3 my-1 rounded-xl focus:outline-none transition-all
              ${activeTab === tab.key ? "bg-gray-900/10 text-gray-900 font-bold" : "text-gray-500 hover:bg-gray-900/10"}`}
            onClick={() => setActiveTab(tab.key)}
            tabIndex={0}
          >
