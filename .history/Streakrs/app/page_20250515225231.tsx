    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex">
      ...
      <aside className={`fixed top-0 left-0 h-screen z-50 transition-all duration-500 ${sidebarOpen ? "w-20 md:w-56" : "w-0 md:w-20"} bg-white/80 backdrop-blur-md shadow-2xl border-r border-gray-200 flex flex-col items-center py-6 overflow-hidden`}
        ...
        <div className="flex flex-col items-center w-full mb-6">
          ...
          {/* Dropdowns (absolute, but inside sidebar) */}
            ...
            <AnimatePresence>
              {notificationsOpen && (
                <motion.div
                  ...
                  className="absolute left-16 top-0 bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-72 z-50 flex flex-col"
            ...
            <AnimatePresence>
              {profileDropdown && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: -10, scale: 0.95 }}
                  className="absolute left-16 top-12 bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-44 z-50 flex flex-col"
                >
        ...
        <nav className="flex-1 flex flex-col gap-1 w-full items-center">
          {/* Main nav items */}
          {[
            { key: "home", icon: "home", label: "Home", color: "text-primary" },
            { key: "explore", icon: "explore", label: "Explore", color: "text-green-600" },
            { key: "gallery", icon: "collections", label: "Collectibles", color: "text-orange-500" },
            { key: "leaderboard", icon: "emoji_events", label: "Leaderboard", color: "text-purple-600" },
          ].map((tab) => (
            <button
              key={tab.key}
              aria-label={tab.label}
              className={`relative group flex items-center w-16 md:w-[90%] py-3 my-1 rounded-xl focus:outline-none transition-all
                ${activeTab === tab.key ? `${tab.color} font-bold bg-white/90 shadow-lg` : "text-gray-500 hover:bg-primary/10"}`}
              onClick={() => setActiveTab(tab.key)}
              tabIndex={0}
              data-tooltip={tab.label}
            >
              {/* Active indicator bar */}
              <span className={`absolute left-0 top-2 bottom-2 w-1 rounded-full ${activeTab === tab.key ? `${tab.color} bg-opacity-80` : "bg-transparent"} transition-all`} />
              <span className={`material-icons text-2xl mx-auto transition-all ${activeTab === tab.key ? "scale-125" : "scale-100"}`}>{tab.icon}</span>
              <span className={`ml-4 text-sm font-medium transition-all ${activeTab === tab.key ? "" : "opacity-80"} hidden md:inline-block`}>{tab.label}</span>
              {/* Tooltip for collapsed sidebar */}
              <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">{tab.label}</span>
            </button>
          ))}
          {/* Divider */}
          <div className="w-10 md:w-[80%] h-px bg-gray-200 my-4 mx-auto" />
          {/* Settings nav item */}
          <button
            aria-label="Settings"
            className={`relative group flex items-center w-16 md:w-[90%] py-3 my-1 rounded-xl focus:outline-none transition-all
              ${activeTab === "settings" ? "text-gray-900 font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:bg-gray-900/10"}`}
            onClick={() => setActiveTab("settings")}
            tabIndex={0}
            data-tooltip="Settings"
          >
            <span className={`absolute left-0 top-2 bottom-2 w-1 rounded-full ${activeTab === "settings" ? "bg-gray-900 bg-opacity-80" : "bg-transparent"} transition-all`} />
            <span className={`material-icons text-2xl mx-auto transition-all ${activeTab === "settings" ? "scale-125" : "scale-100"}`}>settings</span>
            <span className={`ml-4 text-sm font-medium transition-all ${activeTab === "settings" ? "" : "opacity-80"} hidden md:inline-block`}>Settings</span>
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarOpen ? "ml-20 md:ml-56" : "ml-0 md:ml-20"} w-full max-w-7xl mx-auto`}> 
        {/* Remove header, add flex row for welcome card alignment */}
        <div className="flex flex-row justify-between items-start w-full px-4 pt-6 md:pt-8">
          {/* Empty left for alignment (sidebar now has profile/notifications) */}
...
function HomePage({ showWelcomeCardOnly = false }: { showWelcomeCardOnly?: boolean }) {
  const { avatar } = useAvatar();
