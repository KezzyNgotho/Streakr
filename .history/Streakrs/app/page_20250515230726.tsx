  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen z-50 transition-all duration-300
        ${expanded ? "w-56" : "w-16"}
        bg-white/70 backdrop-blur-xl shadow-2xl border-r border-gray-200 flex flex-col
      ...
      <nav className="flex-1 flex flex-col gap-1 w-full items-center">
        {SIDEBAR_NAV.map(tab => (
          <button
            ...
            className={`group relative flex items-center w-12 ${expanded ? "md:w-[90%]" : "w-12"} py-3 my-1 rounded-xl focus:outline-none transition-all
              ${activeTab === tab.key ? "bg-primary/10 text-primary font-bold shadow-lg" : "text-gray-500 hover:bg-primary/10"}`}
            onClick={() => setActiveTab(tab.key)}
            tabIndex={0}
          >
            {/* Active indicator bar */}
            <span className={`absolute left-1 top-2 bottom-2 w-1 rounded-full ${activeTab === tab.key ? "bg-primary bg-opacity-80" : "bg-transparent"} transition-all`} />
        ...
        {SIDEBAR_BOTTOM.map(tab => (
          <button
            ...
            tabIndex={0}
          >
            <span className={`absolute left-1 top-2 bottom-2 w-1 rounded-full ${activeTab === tab.key ? "bg-gray-900 bg-opacity-80" : "bg-transparent"} transition-all`} />
            <span className="text-2xl mx-auto transition-all">{tab.icon}</span>
            {expanded && <span className="ml-4 text-sm font-medium transition-all">{tab.label}</span>}
            {!expanded && (
              <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none">{tab.label}</span>
