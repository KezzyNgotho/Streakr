...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col">
      ...
      {/* Upgraded Bottom Tab Bar with glassmorphism, highlight, and accessibility */}
      <nav
        className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 max-w-lg w-[96vw] flex justify-around items-center py-3 px-3
        bg-white/70 backdrop-blur-md shadow-2xl rounded-3xl border border-gray-200
        transition-all"
        role="tablist"
      ...
      >
        {[
          ...
          { key: "gallery", icon: "collections", label: "Gallery", color: "text-orange-500" },
          { key: "profile", icon: "person", label: "Profile", color: "text-blue-600" },
        ].map((tab, idx) => (
          <button
            key={tab.key}
            aria-label={tab.label}
            aria-current={activeTab === tab.key ? "page" : undefined}
            className={`relative flex flex-col items-center flex-1 px-2 py-1 focus:outline-none group transition-all
              ${activeTab === tab.key ? `${tab.color} font-bold` : "text-gray-500 hover:text-primary/80"}`}
            onClick={() => setActiveTab(tab.key)}
            tabIndex={0}
          >
            {/* Animated pill highlight for active tab */}
            <span
              className={`absolute -top-2 left-1/2 -translate-x-1/2 w-12 h-12 rounded-full z-0
                ${activeTab === tab.key ? "bg-white/80 shadow-lg scale-100" : "scale-75 opacity-0"}
                transition-all duration-300`}
              aria-hidden="true"
            />
            <span className={`material-icons text-2xl z-10 transition-all ${activeTab === tab.key ? "scale-125" : "scale-100"}`}>{tab.icon}</span>
            <span className={`text-xs font-medium z-10 mt-0.5 transition-all ${activeTab === tab.key ? "" : "opacity-80"}`}>{tab.label}</span>
            {/* Notification badge for Profile (stub) */}
            {tab.key === "profile" && (
              <span className="absolute top-2 right-6 w-2 h-2 bg-red-500 rounded-full border-2 border-white animate-pulse" />
            )}
          </button>
        ))}
      </nav>
