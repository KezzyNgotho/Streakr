  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen z-50 transition-all duration-300
        ...
        items-center py-4 px-2 overflow-hidden`}
    ...
    >
      ...
      {/* Nav items */}
      <nav className="flex-1 flex flex-col gap-1 w-full items-center z-10 justify-center">
        {SIDEBAR_NAV.map(tab => (
          <button
            key={tab.key}
          ...
          >
            ...
            <span className={`mx-auto transition-all flex items-center justify-center ${activeTab === tab.key ? 'text-primary' : 'text-gray-400'}`}>{
              // Clone the icon and set size and color
              tab.icon && typeof tab.icon === 'object' && 'type' in tab.icon ?
                // @ts-ignore
                { ...tab.icon, props: { ...tab.icon.props, size: 28, color: activeTab === tab.key ? '#0052FF' : '#A3A3A3' } } :
                tab.icon
            }</span>
