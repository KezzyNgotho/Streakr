            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
              ...
              <div className="w-full max-w-xs mx-auto flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between bg-white rounded-xl shadow p-3">
                  ...
                  <span className="text-xs text-gray-500">{address ? address : 'Not connected'}</span>
                </div>
              </div>
            </div>
          )}
...
function HomePage() {
  ...
  const mobileHeader = (
    <div className="flex md:hidden w-full justify-end items-center px-2 pt-2">
      <HomePageIcons />
    </div>
  );

  const mobileWelcomeCard = (
    <div className="flex md:hidden flex-row items-center gap-2 px-2 py-2 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-2xl shadow-lg w-fit max-w-xs ml-2 mt-2">
      ...
      <div className="flex flex-col min-w-0">
        <div className="text-sm font-bold text-gray-900 truncate">Welcome to <span className="text-primary">Streakrs!</span></div>
  ...
  const desktopTopRow = (
    <div className="hidden md:flex flex-row items-center mt-2 mb-4 w-full gap-2 justify-between px-2 py-2 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-2xl shadow-lg">
      {/* Avatar */}
      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-white flex items-center justify-center text-xl sm:text-3xl shadow border-2 border-white mr-2">
        {avatar}
      </div>
      {/* Welcome text and XP */}
      <div className="flex-1 min-w-0">
        <div className="text-sm sm:text-lg font-bold text-gray-900 truncate">Welcome to <span className="text-primary">Streakrs!</span></div>
        <div className="text-xs text-gray-900 mb-1">Level {level} • {xp} XP</div>
        <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
          <motion.div className="h-full bg-gradient-to-r from-primary to-secondary" animate={{ width: `${xpPercent}%` }} />
        </div>
  ...
  return (
    <>
      ...
      {desktopTopRow}
      {/* Stats Row */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-6 text-center mb-4">
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col items-center w-full max-w-xs mx-auto border border-gray-100 text-xs">
          <span className="text-base sm:text-lg font-bold text-primary">{streaks.length}</span>
          <span className="text-xs text-gray-900">Active</span>
        </div>
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col items-center w-full max-w-xs mx-auto border border-gray-100 text-xs">
          <span className="text-base sm:text-lg font-bold text-secondary">{totalCompletions}</span>
          <span className="text-xs text-gray-900">Completions</span>
        </div>
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col items-center w-full max-w-xs mx-auto border border-gray-100 text-xs">
          <span className="text-base sm:text-lg font-bold text-orange-400">{currentStreak}</span>
          <span className="text-xs text-gray-900">Current</span>
        </div>
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col items-center w-full max-w-xs mx-auto border border-gray-100 text-xs">
          <span className="text-base sm:text-lg font-bold text-green-600">{bestStreak}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-6 mb-8">
        {/* Quick Links */}
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-primary/20 w-full max-w-xs mx-auto text-xs">
        ...
        {/* Tips & Motivation */}
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-secondary/20 w-full max-w-xs mx-auto text-xs">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xl">💡</span>
        ...
        {/* Recent Activity */}
        <div className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-blue-400/20 w-full max-w-xs mx-auto text-xs">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xl">🕒</span>
      ...
      {/* Quick Actions Row */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-6 mb-6">
        <button className="w-full px-4 py-2 rounded-lg bg-primary text-white font-semibold shadow hover:bg-primary/90 transition">+ Add Streak</button>
        {activeStreaks.length > 0 && (
          <button className="w-full px-4 py-2 rounded-lg bg-orange-400 text-white font-semibold shadow hover:bg-orange-500 transition">Claim NFT</button>
        )}
        <button className="w-full px-4 py-2 rounded-lg bg-secondary text-gray-900 font-semibold shadow hover:bg-secondary/80 transition">Share Progress</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-6">
        ...
        <div className="flex flex-col gap-2 sm:gap-6">
          {/* Active Streaks */}
          <section className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-primary/20 text-xs">
            <div className="flex items-center mb-2">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 flex-1">Your Streaks</h3>
              <span className="text-primary font-bold text-base sm:text-lg">{activeStreaks.length}</span>
              <span className="ml-1 text-gray-900">active</span>
              <button className="ml-auto px-3 py-1 rounded bg-secondary text-gray-900 text-xs font-semibold hover:bg-secondary/80 transition shadow">+ Add</button>
            </div>
            <div className="flex flex-col gap-2">
              {activeStreaks.length === 0 && <div className="text-gray-700 text-sm">No streaks yet. Add a habit to get started!</div>}
              <AnimatePresence>
                {activeStreaks.map((streak) => (
                  <motion.div key={streak.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="bg-gradient-to-br from-primary/10 to-secondary/10 rounded-xl px-3 sm:px-4 py-2 sm:py-3 flex flex-col md:flex-row md:items-center gap-2 shadow border border-gray-100">
                    ...
                    <div className="flex items-center gap-2">
                      <button className="px-2 py-1 rounded bg-primary text-white text-xs font-semibold hover:bg-primary/90 transition shadow">+1</button>
                      <button className="px-2 py-1 rounded bg-gray-100 text-gray-900 text-xs font-semibold hover:bg-gray-200 transition shadow">Reset</button>
                      {streak.completed && (
                        <motion.button
          ...
          {completedStreaks.length > 0 && (
            <section className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-green-400/20 text-xs">
              <div className="flex items-center mb-2">
                <h3 className="text-base sm:text-lg font-bold text-green-700 flex-1">Completed Streaks</h3>
                <span className="text-green-600 font-bold text-base sm:text-lg">{completedStreaks.length}</span>
              </div>
              <div className="flex flex-col gap-2">
                {completedStreaks.map((streak) => (
                  <div key={streak.id} className="bg-green-50 rounded-xl px-3 sm:px-4 py-2 sm:py-3 flex flex-col md:flex-row md:items-center gap-2 border border-green-100">
                    <div className="flex-1">
        ...
        <div className="flex flex-col gap-2 sm:gap-6">
          {/* NFT Badge Preview */}
          <section className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-orange-400/20 text-xs">
            <div className="flex items-center mb-2">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 flex-1">NFT Badges</h3>
              <a href="/gallery" className="text-primary text-xs font-semibold">View all</a>
            </div>
            <div className="flex gap-2 sm:gap-3 flex-wrap">
              {activeStreaks.length === 0 && <div className="text-gray-700 text-sm">No NFTs yet. Complete a streak to mint your first badge!</div>}
              {activeStreaks.map((streak) => (
                <motion.div key={streak.id} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center bg-orange-50 rounded-xl shadow p-2 sm:p-3 w-16 sm:w-20 border border-gray-100">
          ...
          <section className="bg-white rounded-md shadow-sm p-2 sm:p-1 flex flex-col gap-1 border border-blue-400/20 text-xs">
            <div className="flex items-center mb-2">
              <h3 className="text-base sm:text-lg font-bold text-blue-700 flex-1">Explore Habits</h3>
            </div>
            <div className="flex flex-wrap gap-2">
        ...
        {notificationsOpen && (
          <div className="absolute right-0 top-12 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800 py-2 w-72 z-50 flex flex-col animate-fade-in">
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n: { id: number; text: string; time: string }) => (
