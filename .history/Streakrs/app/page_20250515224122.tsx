...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex">
      {/* Sleek Sidebar Navigation (with more nav items, animated) */}
      <aside className={`fixed top-0 left-0 h-screen z-50 transition-all duration-500 ${sidebarOpen ? "w-20 md:w-56" : "w-0 md:w-20"} bg-white/80 backdrop-blur-md shadow-2xl border-r border-gray-200 flex flex-col items-center py-6 overflow-hidden`}
        style={{ transitionProperty: 'width,background-color' }}>
        {/* Hamburger for mobile */}
        <button
          className="absolute top-4 left-4 md:hidden z-50 p-2 rounded-full bg-white/80 shadow hover:bg-primary/10 transition"
          onClick={() => setSidebarOpen((open) => !open)}
          aria-label={sidebarOpen ? "Close sidebar" : "Open sidebar"}
        >
          <span className="material-icons text-2xl text-gray-700">{sidebarOpen ? "close" : "menu"}</span>
        </button>
        {/* Logo/App Name */}
        <div className="flex flex-col items-center mb-8 mt-2 w-full select-none">
          <span className="text-3xl font-extrabold text-primary tracking-tight mb-1">🔥</span>
          <span className="text-lg font-bold text-gray-900 hidden md:block">Streakrs</span>
        </div>
        <div className={`flex flex-col gap-4 w-full items-center mt-4 transition-opacity duration-300 ${sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}> 
          {/* Home tab */}
          <button
            aria-label="Home"
            className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
              ${activeTab === "home" ? "text-primary font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:text-primary/80"}`}
            onClick={() => setActiveTab("home")}
            tabIndex={0}
            data-tooltip="Home"
          >
            <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === "home" ? "scale-125" : "scale-100"}`}>home</span>
            <span className={`text-xs font-medium transition-all ${activeTab === "home" ? "" : "opacity-80"} hidden md:block`}>Home</span>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">Home</span>
          </button>
          {/* Explore tab */}
          <button
            aria-label="Explore"
            className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
              ${activeTab === "explore" ? "text-green-600 font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:text-green-600/80"}`}
            onClick={() => setActiveTab("explore")}
            tabIndex={0}
            data-tooltip="Explore"
          >
            <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === "explore" ? "scale-125" : "scale-100"}`}>explore</span>
            <span className={`text-xs font-medium transition-all ${activeTab === "explore" ? "" : "opacity-80"} hidden md:block`}>Explore</span>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">Explore</span>
          </button>
          {/* Collectibles tab */}
          <button
            aria-label="Collectibles"
            className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
              ${activeTab === "gallery" ? "text-orange-500 font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:text-orange-400/80"}`}
            onClick={() => setActiveTab("gallery")}
            tabIndex={0}
            data-tooltip="Collectibles"
          >
            <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === "gallery" ? "scale-125" : "scale-100"}`}>collections</span>
            <span className={`text-xs font-medium transition-all ${activeTab === "gallery" ? "" : "opacity-80"} hidden md:block`}>Collectibles</span>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">Collectibles</span>
          </button>
          {/* Leaderboard tab */}
          <button
            aria-label="Leaderboard"
            className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
              ${activeTab === "leaderboard" ? "text-purple-600 font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:text-purple-600/80"}`}
            onClick={() => setActiveTab("leaderboard")}
            tabIndex={0}
            data-tooltip="Leaderboard"
          >
            <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === "leaderboard" ? "scale-125" : "scale-100"}`}>emoji_events</span>
            <span className={`text-xs font-medium transition-all ${activeTab === "leaderboard" ? "" : "opacity-80"} hidden md:block`}>Leaderboard</span>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">Leaderboard</span>
          </button>
          {/* Settings tab */}
          <button
            aria-label="Settings"
            className={`relative flex flex-col items-center w-16 md:w-full py-3 my-1 rounded-xl focus:outline-none group transition-all
              ${activeTab === "settings" ? "text-gray-900 font-bold bg-white/90 shadow-lg" : "text-gray-500 hover:text-gray-900/80"}`}
            onClick={() => setActiveTab("settings")}
            tabIndex={0}
            data-tooltip="Settings"
          >
            <span className={`material-icons text-2xl mb-1 transition-all ${activeTab === "settings" ? "scale-125" : "scale-100"}`}>settings</span>
            <span className={`text-xs font-medium transition-all ${activeTab === "settings" ? "" : "opacity-80"} hidden md:block`}>Settings</span>
            <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none md:hidden">Settings</span>
          </button>
        </div>
      </aside>
      {/* Main Content shifted right for sidebar */}
      <div className={`flex-1 transition-all duration-300 ${sidebarOpen ? "ml-20 md:ml-56" : "ml-0 md:ml-20"} w-full max-w-7xl mx-auto`}> 
        {/* Top bar with avatar/profile at top right and dropdown */}
        <header className="flex items-center justify-end w-full px-4 py-4 md:py-6 relative">
          <button
            ref={profileBtnRef}
            className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-2xl shadow border-2 border-white hover:ring-2 hover:ring-primary transition"
            aria-label="Profile"
          ...
          <AnimatePresence>
            {profileDropdown && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute right-4 top-16 bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-44 z-50 flex flex-col"
              >
        ...
        <main className="flex-1">
          ...
          {activeTab === "explore" && <div className="flex flex-col items-center justify-center min-h-[60vh] text-center"><span className="material-icons text-5xl text-green-600 mb-2">explore</span><h2 className="text-2xl font-bold mb-2">Explore</h2><p className="text-gray-700">Discover new habits and streaks here soon!</p></div>}
          {activeTab === "leaderboard" && <div className="flex flex-col items-center justify-center min-h-[60vh] text-center"><span className="material-icons text-5xl text-purple-600 mb-2">emoji_events</span><h2 className="text-2xl font-bold mb-2">Leaderboard</h2><p className="text-gray-700">See top streakers and rankings here soon!</p></div>}
          {activeTab === "settings" && <div className="flex flex-col items-center justify-center min-h-[60vh] text-center"><span className="material-icons text-5xl text-gray-900 mb-2">settings</span><h2 className="text-2xl font-bold mb-2">Settings</h2><p className="text-gray-700">Manage your preferences and account here soon!</p></div>}
        </main>
      </div>
    </div>
  );
