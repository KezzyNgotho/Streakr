  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-gray-50 to-gray-100 flex">
      ...
      <aside className={`fixed top-0 left-0 h-screen z-50 transition-all duration-300 ${sidebarOpen ? "w-20 md:w-56" : "w-0 md:w-20"} bg-white/80 backdrop-blur-md shadow-2xl border-r border-gray-200 flex flex-col items-center py-6 overflow-hidden`}>
        ...
        {/* Profile at the top */}
        <button
          className={`flex flex-col items-center w-16 md:w-full mb-8 focus:outline-none group transition-all ${activeTab === "profile" ? "bg-white/90 shadow-lg" : "hover:bg-primary/10"}`}
          onClick={() => setActiveTab("profile")}
          aria-label="Profile"
        >
          <span className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-3xl shadow border-2 border-white mb-1">{useAvatar().avatar}</span>
          <span className="text-xs font-bold text-gray-900 hidden md:block">Profile</span>
        </button>
